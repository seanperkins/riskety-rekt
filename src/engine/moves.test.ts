...
describe("move validation", () => {
  ...
  it("shares the per-origin cap with attacks, and the move wins the collision", () => {
    // Moves validate first on purpose: when an origin is over-committed the
    // reinforcement survives and the attack dies, because a rejected defence
    // loses ground already held while a rejected attack merely fails to gain.
    const { clean, rejections } = validateOrder(state(), {
      factionId: "f1",
      deploys: [],
      attacks: [{ from: "alaska", to: "northwest_territories", count: 3 }],
      moves: [{ from: "alaska", to: "alberta", count: 4 }],
      wagers: [],
      protect: null,
    }, ctx)
    expect(clean.moves).toHaveLength(1)
    expect(clean.attacks).toHaveLength(0)
    expect(rejections.some((r) => r.t === "rejected" && r.field === "attacks")).toBe(true)
  })

  it("counts tonight's deploys toward the origin's cap", () => {
    const { clean, rejections } = validateOrder(state(), {
      factionId: "f1",
      deploys: [{ territory: "alaska", count: 4 }],
      attacks: [],
      moves: [{ from: "alaska", to: "alberta", count: 9 }],
      wagers: [],
      protect: null,
    }, ctx)
    // 6 base + 4 deployed - 1 to hold = 9 may leave.
    expect(rejections).toEqual([])
    expect(clean.moves).toHaveLength(1)
  })
