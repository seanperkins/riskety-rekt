...
**The map is DATA on `GameState.map`, never derived here.** `Territory` carries
`neighbors: TerritoryId[]` directly. `createSeason(seasonId, factions, ids, map?)`
defaults to the engine's own `RISK_MAP` (`map.ts` — classic 42-territory,
6-continent board, bonuses `{na:5, sa:2, eu:5, af:3, as:7, au:2}`; grandfathered
past the region-size band, used only as the golden fixture and test default).
**Production seasons pass `WORLD`** (264 territories, generated by
`scripts/build-shapes.ts` into `src/map/shapes.ts` + `src/map/adjacency.ts`)
instead — the engine can't tell the two apart and computes no adjacency itself
either way. `createSeason` throws unless the dealt id set is EXACTLY the map's
territory set: a territory in the map but not in the deal has no owner, passes
`validateOrder`'s adjacency check, and one troop takes it for free.
...
- **Order validation sits AFTER grant (step 1 before step 2)**: income earned
  this tick is spendable this tick.
...
- **Seniority is the deploy-inflation fix**: a wager locks at its market's
  close, strictly before the tick — so a late deploy can no longer evict a
  locked wager.
...
- Malformed hook returns (negative/fractional amount, unknown faction,
  unparseable `lockedAt`) THROW — the tick refuses; reserves `< 0` also
  throw at the end of `resolve` as a last-resort invariant guard.
...
`validateRules` runs the identical two checks against `RULE_REGISTRY`, a
separate map.
...
Thirteen rules ship, in `RULE_CATALOGUE` (`rules/index.ts`). **Ids are
permanent** — frozen into `tick_context.context.rules` and logged as
`grant.source` — while `name` and `description` are read from the registry at
render time and may change freely. That is why the first three keep bland ids
under witty names. (Three rule FILES were renamed since — `tribute.ts`,
`conscription.ts`, `fortress.ts` — their exported `id`s did not move.)
...
parser), `marketIdsOf` (jobs' settlement set), `pendingWagersOf` (sim's + rules'
read view — `diamond-hands` reads it directly). "Owner-only access" = only
module code interprets the slot, wherever called from. `moduleState` rebuilds
from ACTIVE modules each tick, so a disabled module's slot drops at the next
tick. `DEFAULT_MODULES = ["markets", "irl", "veto"]`.
...
Casualties (`casualties.ts:allocateCasualties`) split pro-rata by force size
with largest-remainder rounding so parts sum exactly to `D` (remainder ties
break on the lower faction id) — full defense against each attacker
independently would break troop conservation. On a mutually-attacked edge the
smaller force dies and the larger continues at `size − 2·smaller`, so a
1-troop feint can no longer void a 100-troop assault.
...
payout(stake, price) = round(stake / clamp(price, 0.1, 0.9) * 1.1)     // round, not floor
```

**One wager per market per faction** — enforced in `validate.ts`, not here;
both sides of one market at `k·p` and `k·(1−p)` would return `1.1k` risk-free.
**Settlement is credit-only** (`settleAll` only ever adds via `credits`, never
subtracts). **Three outcomes, not two**: `outcome` is `"yes" | "no" |
"unsettled"`; unsettled ≥`REFUND_AFTER_TICKS` ticks old refunds at `payout
=== stake` (net-zero) — **`outcome === "unsettled"` is the discriminator**,
since `payout > 0` alone reads a refund as a win. A loss logs `payout = 0`
(event still emitted so the recap shows it). `wagerSettle` carries `stake`,
`faction?`, `marketId?` (optional only because pre-1.1.0 states predate the
fields; this engine always sets both). Price comes from the WAGER when
present (`escrow()`: `w.price ?? slate price`), else the slate.

## Order validation (`validate.ts`)

Shape and legality only; field-level rejection, never throws on player data.
Deploys: count + ownership. Moves/attacks: count, ownership, adjacency (against
`state.map.territories[].neighbors`), target side. Wagers: count, side, slate
membership, **one wager per market**. NOT here anymore: reserve budgeting
